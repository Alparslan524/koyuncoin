...
#if defined(NDEBUG)
# error "Koyuncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x20e066bb0094b2d577cc7ab7eadca6f29c000a72a3be4191df0ce31a74ce7f30");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Koyuncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Koyuncoin Signed Message:\n";
...
{
    // Koyuncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Koyuncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Koyuncoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Koyuncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Koyuncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Koyuncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xef28c8a1ba9b48f636bac6d734f8129ac1f92111fab1c38b8ece81fbd2a860a8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "14/11/2022 -KoyunCoin temelleri atılıyor DENEME 2";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04728c04abd3b53571f5a494524942ab64bc12750b96a100f8e8f7a3b08278822e22e3b0315b388617849941583f27defe308f13c317f5088ea67d2ec3d7df8eaf") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1668454669;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 387959256;

        if (fTestNet)
        {
            block.nTime    = 1668454660;
            block.nNonce   = 387879235;
        }
	...
	//// debug print
        ...
        assert(block.hashMerkleRoot == uint256("0xed9e2570db3854ba8fc2825113a9f7b1b847473eecb70f9cfc125d7381dd58df"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xbd, 0xdc }; // Koyuncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KoyuncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KoyuncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KoyuncoinMiner(CWallet *pwallet)
{
    printf("KoyuncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("koyuncoin-miner");
    ...
    try { loop {
        ...
        printf("Running KoyuncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KoyuncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KoyuncoinMiner, pwallet));
}
